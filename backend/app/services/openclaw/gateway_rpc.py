...
async def steer_session(
    ...
    *,
    ...
    Unlike ``send_message`` there is no ``deliver`` kwarg here —
    ``sessions.steer`` always interrupts AND delivers; offering a
    ``deliver=False`` knob would silently mislead future callers.
    """
    params: dict[str, Any] = {
        "key": session_key,
        "message": message,
