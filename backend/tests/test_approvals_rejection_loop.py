...
@pytest.mark.asyncio
async def test_third_consecutive_rejection_blocks_new_submission() -> None:
...
@pytest.mark.asyncio
async def test_approved_event_resets_the_streak() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
            board, task_ids, _ = await _seed(session)
...
@pytest.mark.asyncio
async def test_unblock_endpoint_clears_the_loop_for_human_user() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
            board, task_ids, _ = await _seed(session)
...
@pytest.mark.asyncio
async def test_worker_agent_cannot_unblock() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
...
@pytest.mark.asyncio
async def test_board_lead_agent_can_unblock() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
            board, task_ids, gateway = await _seed(session)
...
@pytest.mark.asyncio
async def test_loop_guard_is_per_task_not_board_global() -> None:
    engine = await _make_engine()
    try:
        async with await _make_session(engine) as session:
...
@pytest.mark.asyncio
async def test_payload_string_spoof_does_not_unblock() -> None:
...
@pytest.mark.asyncio
async def test_create_approval_rejects_non_pending_status_for_all_actors() -> None:
...
@pytest.mark.asyncio
async def test_multi_task_approval_blocks_only_exceeded_tasks() -> None:
...
@pytest.mark.asyncio
async def test_history_actor_provenance_columns_are_written() -> None:
...
@pytest.mark.asyncio
async def test_reopen_cycle_counts_as_distinct_rejections() -> None:
