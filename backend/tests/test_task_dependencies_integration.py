...
async def test_validate_dependency_update_rejects_self_dependency(
    ...
    board_id = uuid4()
    task_id = uuid4()
    await _seed_board_and_tasks(sqlite_session, board_id=board_id, task_ids=[task_id])
...
@pytest.mark.asyncio
async def test_validate_dependency_update_404s_when_dependency_missing(
    sqlite_session: AsyncSession,
) -> None:
    board_id = uuid4()
    task_id = uuid4()
...
@pytest.mark.asyncio
async def test_validate_dependency_update_detects_cycle(
    sqlite_session: AsyncSession,
) -> None:
    board_id = uuid4()
    a, b = uuid4(), uuid4()
    await _seed_board_and_tasks(sqlite_session, board_id=board_id, task_ids=[a, b])

    # existing edge a -> b
    sqlite_session.add(TaskDependency(board_id=board_id, task_id=a, depends_on_task_id=b))
...
@pytest.mark.asyncio
async def test_dependency_queries_and_replace_and_dependents(
    sqlite_session: AsyncSession,
) -> None:
    board_id = uuid4()
    t1, t2, t3 = uuid4(), uuid4(), uuid4()
    await _seed_board_and_tasks(sqlite_session, board_id=board_id, task_ids=[t1, t2, t3])

    # seed deps: t1 depends on t2 then t3
    sqlite_session.add(TaskDependency(board_id=board_id, task_id=t1, depends_on_task_id=t2))
    sqlite_session.add(TaskDependency(board_id=board_id, task_id=t1, depends_on_task_id=t3))
    await sqlite_session.commit()

    # cover empty input short-circuit
    assert await td.dependency_ids_by_task_id(sqlite_session, board_id=board_id, task_ids=[]) == {}

    deps_map = await td.dependency_ids_by_task_id(
        sqlite_session, board_id=board_id, task_ids=[t1, t2]
    )
    assert deps_map[t1] == [t2, t3]
    assert deps_map.get(t2, []) == []

    # mark t2 done, t3 not
    task2 = (await sqlite_session.exec(select(Task).where(col(Task.id) == t2))).first()
    ...
    status_map = await td.dependency_status_by_id(
        sqlite_session, board_id=board_id, dependency_ids=[t2, t3]
    )
    assert status_map[t2] == td.DONE_STATUS
    assert status_map[t3] != td.DONE_STATUS

    blocked = await td.blocked_by_for_task(sqlite_session, board_id=board_id, task_id=t1)
    assert blocked == [t3]

    # cover early return when no deps provided
    assert (
        await td.blocked_by_for_task(
            sqlite_session, board_id=board_id, task_id=t1, dependency_ids=[]
        )
        == []
    )
    ...
    dependents = await td.dependent_task_ids(
        sqlite_session, board_id=board_id, dependency_task_id=t3
    )
    assert dependents == [t1]

    # also exercise explicit dependency_ids passed
    blocked2 = await td.blocked_by_for_task(
        sqlite_session, board_id=board_id, task_id=t1, dependency_ids=[t3]
    )
    assert blocked2 == [t3]
